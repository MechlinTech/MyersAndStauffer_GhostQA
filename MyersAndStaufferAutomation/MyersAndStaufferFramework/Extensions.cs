...
namespace MyersAndStaufferFramework
{
    public static class Extensions
    {
        ...
        public static string FullExceptionMessage(this Exception ex)
        {
            ...
            {
                return $"\"{ex.Message}\" -> \"{FullExceptionMessage(ex.InnerException)}\"";
            }
        }
            ...
            return Regex.Split(str ?? "", " ").ToList()
                ...
                .Aggregate((memo, w) =>
                {
                    ...
                    else
                    {
                        ...
                        if (memo.Current.Length <= numberOfChars)
                            return new { Continue = ok, Current = ok ? term : memo.Current };
                        else
                            return new { Continue = false, Current = memo.Current };
                    }
                })
                .Current
                .Left(numberOfChars);
        ...
        {
            IDictionary<string, object> node = source as IDictionary<string, object>;

            if (node == null)
            {
                if (source != null)
                {
                    throw new InvalidOperationException($"Passed object does not implement IDictionary");
                }
